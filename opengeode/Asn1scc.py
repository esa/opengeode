...
def parse_asn1(*files, **options):
    ...
    try:
        for each in file_list:
            ...
            # The reason we do that is because of the optional flags
            # set by the caller to asn1scc, in particular the RenamePolicy
            # is different between the C and Ada backends. So if we try
            # here to reload a cached module that was originally generated
            # with different options, there will eventually be compilation
            # errors.
            # Adding the hash to the file path is not really solving the
            # issue, as if we first target C then we change to target Ada,
            # the file will be the same. So specifically for C and Ada,
            # we need to have different filenames.
            # Alternatively if more cases arise, we could instead hash
            # the command line to asn1scc...
    ...
    # Depending on command line options (--toC or --toAda), prefix the file
    # name (see above - they use different compilation options)
    prefix = "c_" if "--toC" in sys.argv else ""
    new_hash = prefix + new_hash
