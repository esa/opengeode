...
from .genericSymbols import HorizontalSymbol, VerticalSymbol, Comment, Symbol
...
class Alternative(Decision):
    ''' The symbol is different but for the rest this class
    is identical to the Decision '''
    common_name = 'alternative'
    def set_shape(self, width, height):
        ''' Define polygon points to draw the symbol '''
        if width != self.width or height != self.height:
            path = QPainterPath()
            ...
            path.lineTo(width / 2, 0)
            self.setPath(path)
            Symbol.set_shape(self, width, height)
...
class State(VerticalSymbol):
    ...
    def double_click(self):
        ...
        # We have to check first in the current scene as we may not be at
        # partition level, in we are in a sub-sub-scene (more than one level
        # of state nesting). Then if nothing is found, we have to look
        ...
        for each, value in self.scene().composite_states.items():
            if str(self).split()[0].lower() == str(each):
                ...
                return
        # Nothing found in the current scene, check in all partitions
        try:
