...
def setMsgQ():
    global tmId
    tmId = PythonController.i_telemetry


def setUDP():
    global tmId
    from _InterfaceEnum import i_orchestrator_PI_telemetry
    tmId = i_orchestrator_PI_telemetry


def telemetry(tm_ptr, size):
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ telemetry ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received telemetry with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected telemetry (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for telemetry' % str(msgId)
            Q.task_done()
