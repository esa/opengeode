...
def setMsgQ():
    global tmId
    tmId = PythonController.i_peek_list


def setUDP():
    global tmId
    from _InterfaceEnum import i_orchestrator_PI_peek_list
    tmId = i_orchestrator_PI_peek_list


def peek_list(tm_ptr, size):
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.TASTE_Peek_id_list_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def decode_ACN(ACN_encodedBuffer):
    ...
    decoded_value = ASN1.TASTE_Peek_id_list()
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.TASTE_Peek_id_list_REQUIRED_BYTES_FOR_ENCODING)
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ peek-list ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received peek-list with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected peek-list (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print 'Received other (%s), but still waiting for peek-list' % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type TASTE_Peek_id_list
    # peek_list = ASN1.TASTE_Peek_id_list()
    return None

def typeInstance():
    # Create an intance of an ASN.1 ctype
    return ASN1.TASTE_Peek_id_list()
